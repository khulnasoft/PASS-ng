...
if [ "$kadmin_exists" ] || [ "$klist_exists" ] || [ "$kinit_exists" ] || [ "$PSTORAGE_KERBEROS" ] || [ "$DEBUG" ]; then
  ...
  print_info "http://book.khulnasoft.com/linux-hardening/privilege-escalation/linux-active-directory"
...
if ([ "$screensess" ] || [ "$screensess2" ] || [ "$DEBUG" ]) && ! [ "$SEARCH_IN_FOLDER" ]; then
  ...
  print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation#open-shell-sessions"
...
if [ "$PSTORAGE_KCPASSWORD" ] || [ "$DEBUG" ]; then
  ...
  print_info "https://book.khulnasoft.com/macos/macos-security-and-privilege-escalation#kcpassword"
...
if [ "$(command -v gitlab-rails)" ] || [ "$(command -v gitlab-backup)" ] || [ "$PSTORAGE_GITLAB" ] || [ "$DEBUG" ]; then
  ...
  if [ "$(command -v gitlab-rails)" ]; then
    ...
    gitlab-rails runner 'User.where.not(username: "passsssssss").each { |u| pp u.attributes }' | sed -${E} "s,email|password,${SED_RED},"
    ...
    echo "Alternatively, you could change the password of any user by running: gitlab-rails runner 'user = User.find_by(email: \"admin@example.com\"); user.password = \"pass_pass_pass\"; user.password_confirmation = \"pass_pass_pass\"; user.save!'"
    echo ""
  fi
...
if ! [ "$SEARCH_IN_FOLDER" ]; then
  ...
  if [ "$containerd" ] || [ "$DEBUG" ]; then
    ...
    print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation/containerd-ctr-privilege-escalation"
    if [ "$containerd" ]; then
...
if ! [ "$SEARCH_IN_FOLDER" ]; then
  ...
  if [ "$runc" ] || [ "$DEBUG" ]; then
    ...
    print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation/runc-privilege-escalation"
    if [ "$runc" ]; then
...
if [ "$PSTORAGE_DOCKER" ] || [ "$DEBUG" ]; then
  ...
  print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation/docker-breakout/docker-breakout-privilege-escalation"
...
pass{Interesting logs}

pass{Windows}

pass{Other Interesting}
