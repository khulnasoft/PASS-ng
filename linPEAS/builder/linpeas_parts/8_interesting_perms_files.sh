...
print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation#sudo-and-suid"
if ! [ "$STRINGS" ]; then
...
if ! [ "$SEARCH_IN_FOLDER" ] && ! [ "$IAMROOT" ]; then
  ...
  print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation#ld.so"
...
if ! [ "$SEARCH_IN_FOLDER" ]; then
  ...
  print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation#capabilities"
...
print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation#acls"
if ! [ "$SEARCH_IN_FOLDER" ]; then
...
#  print_info "https://book.khulnasoft.com/macos/macos-security-and-privilege-escalation#resource-forks-or-macos-ads"
...
if ! [ "$SEARCH_IN_FOLDER" ]; then
  ...
  print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation#profiles-files"
...
print_2title "Permissions in init, init.d, systemd, and rc.d"
  print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation#init-init-d-systemd-and-rc-d"
  if [ ! "$MACPEAS" ] && ! [ "$IAMROOT" ]; then #Those folders don´t exist on a MacOS
...
if ! [ "$IAMROOT" ]; then
  ...
  print_info "https://book.khulnasoft.com/linux-hardening/privilege-escalation#writable-files"
