...
${NC}This tool enum and search possible misconfigurations$DG (known vulns, user, processes and file permissions, special file permissions, readable/writable files, bruteforce other users(top1000pwds), passwords...)$NC inside the host and highlight possible misconfigurations with colors.
      ${GREEN}  Checks:
        ...
        ${YELLOW}    -o${BLUE} Only execute selected checks (pass{CHECKS}). Select a comma separated list.
...
print_support () {
  printf """
    ${GREEN}/---------------------------------------------------------------------------------\\
    |                             ${BLUE}Do you like PASS?${GREEN}                                  |
    |---------------------------------------------------------------------------------|
    |         ${YELLOW}Get the latest version${GREEN}    :     ${RED}https://github.com/sponsors/khulnasoft${GREEN} |
    |         ${YELLOW}Follow on Twitter${GREEN}         :     ${RED}@khulnasoft${GREEN}                        |
...
printf ${BLUE}"          $SCRIPTNAME-$VERSION ${YELLOW}by khulnasoft\n"$NC;
echo ""
printf ${YELLOW}"ADVISORY: ${BLUE}$ADVISORY\n$NC"
echo ""
printf ${BLUE}"Linux Privesc Checklist: ${YELLOW}https://book.khulnasoft.com/linux-hardening/linux-privilege-escalation-checklist\n"$NC
...
sidVB='pass{SUIDVB1_HERE}'
sidVB2='pass{SUIDVB2_HERE}'
cfuncs='file|free|main|more|read|split|write'

sudoVB1=" \*|env_keep\W*\+=.*LD_PRELOAD|env_keep\W*\+=.*LD_LIBRARY_PATH|pass{SUDOVB1_HERE}"
sudoVB2="pass{SUDOVB2_HERE}"
...
cap_setuid:pass{CAP_SETUID_HERE} \
cap_setgid:pass{CAP_SETGID_HERE} \
...
INT_HIDDEN_FILES="pass{INT_HIDDEN_FILES}"

FAT_LINPEAS_AMICONTAINED="pass{AMICONTAINED}"
FAT_LINPEAS_GITLEAKS_LINUX="pass{GITLEAKS_LINUX}"
FAT_LINPEAS_GITLEAKS_MACOS="pass{GITLEAKS_MACOS}"
...
#----------) Network functions (----------#
###########################################
# Adapted from https://github.com/khulnasoft/bashReconScan/blob/master/brs.sh
