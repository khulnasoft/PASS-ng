...
from .passRecord import PEASRecord
...
class LinpeasBuilder:
    ...
    def build(self):
        ...
        pass_marks = self.__get_pass_marks()
        for i,mark in enumerate(pass_marks):
            for j in range(i+1,len(pass_marks)):
                assert mark != pass_marks[j], f"Found repeated pass mark: {mark}"

        print("[+] Building autocheck sections...")
        sections = self.__generate_sections()
        for section_name, bash_lines in sections.items():
            mark = "pass{"+section_name+"}"
            if mark in pass_marks:
        ...
        #Check that there arent pass marks left in linpeas
        pass_marks = self.__get_pass_marks()
        assert len(pass_marks) == 0, f"There are pass marks left: {', '.join(pass_marks)}"
        
        #Check for empty seds
        assert 'sed -${E} "s,,' not in self.linpeas_sh
    
    def __get_pass_marks(self):
        return re.findall(r'pass\{[\w\-\._ ]*\}', self.linpeas_sh)
